...
local function debug_hook(event, line)
  ...
  elseif event == "line" then
    ...
    elseif not status and res then
      error(res or "Stew's Error in MobDebug 522", 2) -- report any other (internal) errors back to the application
    end
    ...
    elseif command == "EXEC" then
      ...
      if chunk then 
        ...
        if status then
          ...
          server:send("401 Error in Expression " .. (res and #res or 0) .. "\n")
          server:send(res or "Bad Res" )
        end
      else
        server:send("400 Bad Request\n")
      end
